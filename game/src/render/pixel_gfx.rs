...
/// Fill a circle with radial gradient.
...
pub fn fill_triangle(
    ...
    for y in min_y..=max_y {
        ...
        if xs.len() >= 2 {
            ...
            for x in left.ceil() as i32..=right.floor() as i32 {
...
/// Draw a flame/fire effect at position.
pub fn draw_flame(img: &mut RgbaImage, cx: f32, cy: f32, size: f32, frame: u64) {
    let phase = (frame as f32 * 0.3).sin() * 0.3 + 0.7;
    let h = (size * phase) as i32;
    for dy in 0..h {
        let t = dy as f32 / h as f32;
        let width = (size * (1.0 - t) * 0.6) as i32;
        let wobble = ((frame as f32 * 0.5 + dy as f32 * 0.2).sin() * 2.0) as i32;
        let r = 255;
        let g = (255.0 * (1.0 - t * 0.7)) as u8;
        let b = (80.0 * (1.0 - t)) as u8;
        let a = (255.0 * (1.0 - t * 0.5)) as u8;
        for dx in -width..=width {
            blend_pixel(img, cx as i32 + dx + wobble, cy as i32 - dy, [r, g, b, a]);
        }
    }
}

/// Draw a glowing point (additive-style).
pub fn draw_glow(img: &mut RgbaImage, cx: f32, cy: f32, radius: f32, color: [u8; 3]) {
    let r_ceil = radius.ceil() as i32 + 1;
    for dy in -r_ceil..=r_ceil {
        for dx in -r_ceil..=r_ceil {
            ...
            if dist < radius {
                ...
                let a = (intensity * intensity * 180.0) as u8;
                blend_pixel(img, cx as i32 + dx, cy as i32 + dy, [color[0], color[1], color[2], a]);
            }
        }
    }
}
...
/// Returns the width in pixels that text would occupy at given scale.
pub fn text_width(text: &str, scale: u32) -> i32 {
    let len = text.len() as i32;
    if len == 0 { return 0; }
    len * 6 * scale as i32 - scale as i32
}
